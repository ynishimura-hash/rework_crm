...
export async function POST(request: Request) {
    try {
        ...
        // 2. 請求書のステータスを "issue" に更新する
        const updateData = {
            company_id: companyId,
            invoice_status: 'issue'
        };

        const updateResponse = await fetch(`https://api.freee.co.jp/iv/invoices/${invoice_id}`, {
            method: 'PUT',
            headers: {
        ...
        if (!updateResponse.ok) {
            console.error("freee API update invoice error:", updateResult);
            // 既にissueになっているなどのエラーはスキップできるようにハンドリング
            if (updateResult.errors?.[0]?.messages?.[0]?.includes('invoice_status')) {
        ...
        console.error('Error issuing freee invoice:', error);
